import org.rumbledb.langserver.JsoniqLanguageServer;
...
/**
 * Launcher for JSONiq language server.
 */
public class StdioLauncher {
    ...
    private static void startServer(InputStream in, OutputStream out) throws ExecutionException, InterruptedException {
        // Initialize the JSONiqLanguageServer
        JsoniqLanguageServer jsoniqLanguageServer = new JsoniqLanguageServer();
        // Create JSON RPC launcher for JSONiqLanguageServer instance.
        Launcher<LanguageClient> launcher = LSPLauncher.createServerLauncher(jsoniqLanguageServer, in, out);

        // Get the client that request to launch the LS.
        LanguageClient client = launcher.getRemoteProxy();

        // Set the client to language server
        jsoniqLanguageServer.connect(client);
